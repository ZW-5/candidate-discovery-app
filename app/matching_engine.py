...
def generate_mock_data(data_dir='data'):
    ...
    feedback = []
    for app in apps:
        text = random.choice(strong if random.random() < 0.3 else weak)
    ...
    pd.DataFrame(feedback).to_csv(f"{data_dir}/interview_feedback.csv", index=False)
    ...
    pd.DataFrame(resumes).to_csv(f"{data_dir}/resumes.csv", index=False)
    ...
    for req in reqs:
        jds.append({
            ...
            "job_description": faker.paragraph(nb_sentences=6)
        })
    pd.DataFrame(jds).to_csv(f"{data_dir}/job_descriptions.csv", index=False)
...
def match_candidates(apps, feedback, reqs, resumes, jds):
    ...
    # Keep only high-quality feedback
    ...
    for _, row in not_hired.iterrows():
        # Feedback similarity among peers
        peers = hired[
        ...
        # Only include if at least one strong signal
        if score_fb >= 0.7 or score_jd >= 0.7:
            index_score = round((score_fb + score_jd) / 2, 3)
            best_peer   = peers.iloc[idx_fb]
            ...
            results.append({
                ...
                "matched_to_hired_id":     best_peer["applicant_id"],
                "matched_feedback":        best_peer["feedback_text"],
                "feedback_similarity":     score_fb,
                "resume_jd_similarity":    score_jd,
                "rediscovery_index_score": index_score
            })

    # Ensure output folder and write
